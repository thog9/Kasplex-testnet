...
LANG = {
    'vi': {
        'title': 'WRAP KAS - KASPLEX TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'wrap_success': 'Wrap {amount} KAS → WKAS thành công!',
        'unwrap_success': 'Unwrap {amount} WKAS → KAS thành công!',
        'failure': 'Thất bại',
        'address': 'Địa chỉ',
        'amount': 'Số dư',
        'gas': 'Gas',
        ...
        'block': 'Khối',
        'balance': 'Số dư: {kas:.6f} KAS | WKAS: {wkas:.6f}',
        'balance_info': 'Số dư',
        'pausing': 'Tạm nghỉ',
        'seconds': 'giây',
        'completed': 'HOÀN THÀNH: {successful}/{total} GIAO DỊCH THÀNH CÔNG',
        'connect_success': 'Thành công: Đã kết nối mạng Kasplex Testnet',
        ...
        'input_amount': 'Nhập số KAS (0.000001 - 999): ',
        ...
        'choose_action': 'Chọn hành động [1: KAS → WKAS | 2: WKAS → KAS | 3: Wrap ←→ Unwrap]: ',
        'wrap_only': 'Wrap KAS → WKAS',
        'unwrap_only': 'Unwrap WKAS → KAS',
    ...
    'en': {
        ...
        'wrap_success': 'Successfully wrapped {amount} KAS → WKAS!',
        'unwrap_success': 'Successfully unwrapped {amount} WKAS → KAS!',
        'failure': 'Failed',
        'address': 'Address',
        'amount': 'Balance',
        'gas': 'Gas',
        ...
        'total_cost': 'Total Cost',
        'block': 'Block',
        'balance': 'Balance: {kas:.6f} KAS | WKAS: {wkas:.6f}',
        'balance_info': 'Balance',
        'pausing': 'Pausing',
        'seconds': 'seconds',
        'completed': 'COMPLETED: {successful}/{total} TRANSACTIONS SUCCESSFUL',
        'connect_success': 'Success: Connected to Kasplex Testnet',
        ...
        'input_amount': 'Enter KAS amount (0.000001 - 999): ',
        ...
        'choose_action': 'Choose action [1: KAS → WKAS | 2: WKAS → KAS | 3: Wrap ←→ Unwrap]: ',
        'wrap_only': 'Wrap KAS → WKAS',
        'unwrap_only': 'Unwrap WKAS → KAS',
...
def display_all_wallets_balances(w3: Web3, private_keys: List[str], language: str = 'en'):
    ...
    print(f"{Fore.CYAN}  Wallet | {'KAS':<10} | {'WKAS':<10}{Style.RESET_ALL}")
    ...
    for i, key in enumerate(private_keys, 1):
        ...
        kas_balance = check_balance(w3, address, None, language)
        wkas_balance = check_balance(w3, address, contract, language)
        print(f"{Fore.YELLOW}  {i:<6} | {kas_balance:>10.6f} | {wkas_balance:>10.6f}{Style.RESET_ALL}")
    
    print()

def display_token_balance_wallet(w3: Web3, address: str, contract, language: str = 'en'):
    kas_balance = check_balance(w3, address, None, language)
    wkas_balance = check_balance(w3, address, contract, language)
    print_message(f"    - {LANG[language]['balance'].format(kas=kas_balance, wkas=wkas_balance)}", Fore.YELLOW)
...
def connect_web3(language: str = 'en'):
    try:
        w3 = Web3(Web3.HTTPProvider(KASPLEX_TESTNET_RPC_URL))
...
def check_balance(w3: Web3, address: str, contract=None, language: str = 'en') -> float:
    if contract is None:  # Native KAS
        try:
...
async def wrap_kas(w3: Web3, private_key: str, amount: int, proxy: str, language: str) -> bool:
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            kas_balance = check_balance(w3, sender_address, None, language)
            wkas_balance = check_balance(w3, sender_address, contract, language)
            print_message(f"    - {LANG[language]['balance'].format(kas=kas_balance, wkas=wkas_balance)}", Fore.YELLOW)
            if kas_balance < CONFIG['MINIMUM_BALANCE'] or kas_balance < w3.from_wei(amount, 'ether'):
                print_message(f"  ✖ {LANG[language]['insufficient_balance'].format(balance=kas_balance, symbol='KAS', required=max(CONFIG['MINIMUM_BALANCE'], w3.from_wei(amount, 'ether')))}", Fore.RED)
                return False
            ...
            if receipt is None:
                ...
                print_message(f"    - {LANG[language]['total_cost']}: {total_cost:.12f} KAS", Fore.YELLOW)
                kas_balance = check_balance(w3, sender_address, None, language)
                wkas_balance = check_balance(w3, sender_address, contract, language)
                print_message(f"    - {LANG[language]['balance'].format(kas=kas_balance, wkas=wkas_balance)}", Fore.YELLOW)
                return True
...
async def unwrap_wkas(w3: Web3, private_key: str, amount: int, proxy: str, language: str) -> bool:
    account = Account.from_key(private_key)
    sender_address = account.address
    contract = w3.eth.contract(address=Web3.to_checksum_address(WKAS_CONTRACT), abi=contract_abi)
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if kas_balance < CONFIG['MINIMUM_BALANCE']:
                print_message(f"  ✖ {LANG[language]['insufficient_balance'].format(balance=kas_balance, symbol='KAS', required=CONFIG['MINIMUM_BALANCE'])}", Fore.RED)
                return False
            if wkas_balance * (10 ** 18) < amount:
                print_message(f"  ✖ {LANG[language]['insufficient_balance'].format(balance=wkas_balance, symbol='WKAS', required=w3.from_wei(amount, 'ether'))}", Fore.RED)
                return False
    ...
    try:
        for cycle in range(1, cycles + 1):
            ...
            if action == '1':  # Wrap only
                ...
                if await wrap_kas(w3, private_key, amount_wei, proxy, language):
                    successful_txs += 1
            
            elif action == '2':  # Unwrap only
                print_message(f"> {LANG[language]['unwrap_only']}", Fore.CYAN)
                if await unwrap_wkas(w3, private_key, amount_wei, proxy, language):
                    successful_txs += 1
...
async def run_wrap(language: str = 'en'):
    print()
