...
# Constants
NETWORK_URL = "https://rpc.kasplextest.xyz"
CHAIN_ID = 167012
...
CONFIG = {
    ...
    "MAX_CONCURRENCY": 5,
    "MAX_RETRIES": 3,
    "MINIMUM_BALANCE": 0.000001  # KAS
}

# Từ vựng song ngữ
LANG = {
    'vi': {
        'title': '✨ GỬI GIAO DỊCH - KASPLEX TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'enter_amount': '✦ NHẬP SỐ LƯỢNG KAS',
        'amount_prompt': 'Số lượng KAS (mặc định 0.000001, tối đa 999): ',
        'amount_unit': 'KAS',
        ...
        'insufficient_balance': 'Số dư không đủ (cần ít nhất {required:.6f} KAS cho giao dịch)',
    ...
    'en': {
        'title': '✨ SEND TRANSACTION - KASPLEX TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
        ...
        'enter_amount': '✦ ENTER AMOUNT OF KAS',
        'amount_prompt': 'Amount of KAS (default 0.000001, max 999): ',
        'amount_unit': 'KAS',
        ...
        'insufficient_balance': 'Insufficient balance (need at least {required:.6f} KAS for transaction)',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {balance:.6f} KAS{Style.RESET_ALL}")
                return False
...
async def process_wallet(index: int, profile_num: int, private_key: str, proxy: str, w3: Web3, choice: str, tx_count: int, amount: float, addresses: list, language: str):
    ...
    if eth_balance < CONFIG['MINIMUM_BALANCE']:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=CONFIG['MINIMUM_BALANCE'])}: {eth_balance:.6f} KAS{Style.RESET_ALL}")
        return 0
